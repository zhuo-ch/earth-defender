...
class Game {
    ...
    switch (this.nav.currentGun) {
      case 'Rocket':
        if (this.canShoot && endPos[1] < 440) {
          this.projectiles.push(new Rocket({pos, vel, endPos, nav}));
      ...
      case 'ClusterGun':
        if (this.canShoot && endPos[1] < 440) {
          this.projectiles.push(new ClusterGun({pos, vel, endPos, nav}));
      ...
      case 'GravityGun':
        if (this.canShoot) {
          this.projectiles.push(new GravityGun({pos, vel, endPos, nav}));
      ...
      case 'Laser':
        if (this.canShoot) {
          this.projectiles.push(new Laser({pos, game: this, endPos, bounds: this.bounds}));
          this.canShoot = false;
          this.nav.startLaser();
        }
        break;
  ...
  addMissiles() {
    ...
    for (let i = 0; i < missilesCount; i++) {
      this.projectiles.push(new Missile({ game: this }));
    }
  };
